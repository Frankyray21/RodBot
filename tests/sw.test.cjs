...
function harness() {
  ...
  return {
    ...
    async request(path, { mode = 'cors', method = 'GET' } = {}) {
      let promise;
      handlers.fetch({
        request: { url: keyOf(path), mode, method },
...
test('activation deletes only old RodBot caches', async () => {
  const h = harness();
  for (const name of [CURRENT, 'rodbot-formation-v1.40.0', 'tms-v2.08', 'forage-v1', 'rodbot-model-3d']) {
    h.seed(name, './index.html', name);
  }
  await h.lifecycle('activate');
  assert.deepEqual(h.deleted, ['rodbot-formation-v1.40.0']);
  assert.equal(h.claimed, true);
  assert.equal(h.stores.size, 4);
});
...
test('asset lookup stays within the current RodBot cache', async () => {
  const h = harness();
