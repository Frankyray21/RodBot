...
test('wheel is radius-relative, fine nearby, and leaves target, yaw and field of view unchanged',()=>{
 for(const r of [.5,1,3,6]){const h=harness(r);const e=h.send('wheel',{deltaY:-100,deltaMode:0});h.settle();assert(e.defaultPrevented);const ratio=h.model.orbit.radius/r;assert(ratio>.92&&ratio<.98);assert.equal(h.model.fieldOfView,'30deg');assert.equal(h.model.cameraTarget,'unchanged');assert.equal(h.model.orbit.theta,-.4);assert.equal(h.model.zoomSensitivity,0);}
...
test('wheel bursts accumulate instead of restarting from an unfinished frame',()=>{
 const single=harness(),burst=harness();single.send('wheel',{deltaY:-100});for(let i=0;i<4;i++)burst.send('wheel',{deltaY:-100});single.settle();burst.settle();assert(burst.model.orbit.radius<single.model.orbit.radius-.05);assert.equal(burst.interactions,4);
});
