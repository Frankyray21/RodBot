...
var APP_VERSION_DATE = '24 SEPT. 2026';
...
class Component extends DCLogic {
    ...
    },
    {
      ...
      sections:[
        { title:"Guide de dépannage, les cas fréquents", page:67, blocks:[
          ...
          {t:"p", text:"Assistance MEDATech disponible au besoin."} ]},
  ...
  constructor(props){
    ...
    // Reprise après interruption : si la tablette s'est fermée en pleine leçon
    // ou en plein quiz (batterie, redémarrage), on restaure l'endroit exact et
    // les réponses déjà données. Instantané valide 24 h.
    try {
      var rz = JSON.parse(localStorage.getItem("rodbot_reprise_v1")||"null");
      if (rz && rz.ts && (Date.now()-rz.ts) < 86400000 &&
        ...
        if (rzView==="quiz") {
          ...
          this.state.qChecked = !!rz.qChecked;
          this.state.qResults = Array.isArray(rz.qResults) ? rz.qResults.slice(0, qn) : [];
          this.state.graded = !!rz.graded;
    ...
    // forcé quand l'app passe en arrière-plan ou se ferme.
    window.addEventListener("pagehide", ()=>this.resumeFlush());
    document.addEventListener("visibilitychange", ()=>{ if(document.hidden) this.resumeFlush(); });
  }

  setState(patch, cb){
    super.setState(patch, cb);
    clearTimeout(this._rzT);
    this._rzT = setTimeout(()=>this.resumeFlush(), 600);
  }
  resumeFlush(){
    try {
      var S = this.state;
      if (S.view!=="module" && S.view!=="quiz") { localStorage.removeItem("rodbot_reprise_v1"); return; }
      localStorage.setItem("rodbot_reprise_v1", JSON.stringify({
        ts:Date.now(), v:APP_VERSION, view:S.view, activeId:S.activeId, openKey:S.openKey,
        qIdx:S.qIdx, qSel:S.qSel, qChecked:S.qChecked, qResults:S.qResults,
        graded:S.graded, lastScore:S.lastScore, lastPassed:S.lastPassed
      }));
    } catch(e){}
  }
    ...
    var step = ()=>{
      ...
      try{
        ...
        if(ph){
          ...
          var header = ROOT.querySelector('.rb-topbar');
          var inset = (header ? header.offsetHeight : 64) + 12;
          if(r.top < inset || r.bottom > vh * 0.46){
            var y = (window.pageYOffset || document.documentElement.scrollTop || 0) + r.top - inset;
  ...
  renderVals(){
    ...
    if(S.home==="formation"){
      ...
      homeTocData=[
        ...
        ["machine3d","02",this.tr("La machine en 3D","The machine in 3D")],
        ["safety","03",this.tr("Sécurité","Safety")],
        ["practice","04",this.tr("Pratique","Practice")],
        ["documents","05",this.tr("Documents","Documents")]
      ];
    } else {
      homeTocData=[
    ...
    base.pgKlaxon = { n:this.mp(16), open:()=>this.openManual(this.mp(16)) };
    base.pgValves = { n:this.mp(50), open:()=>this.openManual(this.mp(50)) };
    ...
    base.appVersionDate = this.tr(APP_VERSION_DATE, "SEP 24, 2026");
  ...
  },
  {
    ...
    sections:[
      { title:"Troubleshooting guide, common cases", page:67, blocks:[
        ...
        {t:"p", text:"MEDATech support available if needed."} ]},
...
ENRICH["0-0"] = {
  ...
  figures: [{"page": 1, "cap": "Le RodBot LP : mât robotisé, grappin, bac à tiges et chenilles (couverture du manuel)."}]
};
