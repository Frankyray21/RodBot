...
**L'accueil affiche cette même réplique en direct** (`hero3d.js`) : la machine se tourne au doigt, se zoome avec `+` et `−` ou par pincement, et revient à sa vue de départ avec « Recentrer ». La photo de la machine reste affichée pendant le chargement, et reste seule si la 3D ne peut pas s'ouvrir.

Le modèle, le moteur d'affichage et les décodeurs sont servis depuis ce dépôt. Aucun compte ChatGPT n'est requis. Le même fichier GLB sert à l'accueil et à la page 3D : il n'est téléchargé qu'une fois, puis conservé hors ligne. Les quiz, les fiches du scan et les attestations conservent leur fonctionnement.
