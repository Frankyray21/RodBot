/* Real-browser controls and canvas renders. Software CI uses a modest pixel
 * ratio, while preserving the application's geometry, controls and lighting. */
...
(async()=>{
 ...
 try{
  ...
  const fresh=async()=>{
   ...
   context=await browser.newContext({viewport:{width:1280,height:960},deviceScaleFactor:.75,serviceWorkers:'block'});
  ...
  const capture=async(file,type='png',selector='#viewer',jpegCopy=null)=>{
   const data=await page.evaluate(async({selector})=>{
    const m=document.querySelector(selector);if(!m.loaded)throw Error('Model not loaded');
    ...
    return {png:canvas.toDataURL('image/png'),jpeg:canvas.toDataURL('image/jpeg',.92)};
   },{selector});
   fs.writeFileSync(file,Buffer.from(data[type].split(',')[1],'base64'));
   if(jpegCopy)fs.writeFileSync(jpegCopy,Buffer.from(data.jpeg.split(',')[1],'base64'));
  };
  await fresh();await loadFree();
  results.stage='overview';save();
  await pose('-35deg 70deg 110%','auto auto auto');assert((await page.$eval('#viewer',m=>m.getCameraOrbit().radius))>1);
  const poster=path.join(root,'3d/assets/rodbot-v11-hydraulics-poster.jpg');
  await capture(poster,'jpeg');fs.copyFileSync(poster,path.join(out,'overview.jpg'));
  results.stage='detail';save();
  await pose('-15deg 72deg 2m','-0.45m 2.02m 0.27m');
  await capture(path.join(out,'hydraulics-after.png'),'png','#viewer',path.join(root,'3d/assets/rodbot-v11-hydraulics-detail.jpg'));
